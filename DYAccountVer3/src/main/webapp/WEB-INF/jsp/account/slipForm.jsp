...
<script>
        ...
        function deleteSlip() {
            $.ajax({
                type: "DELETE",
                url: "${pageContext.request.contextPath}/account/Slip",
                data: {
        ...
        function createPdf() {
        	$("#sendPdf").prop("disabled",false);
        	   window.open("${pageContext.request.contextPath}/base/slipDetailIreport.do?slipNo=" + $(slipGrid).jqGrid('getGridParam',"selrow"), "window", "width=1600, height=1000");
        	   
        }
        //EXCEL로 보기
        function createExcel(){
        	
        	location.href="${pageContext.request.contextPath}/base/slipExcel.do?from=" + $("#from").val()+"&to="+$("#to").val();
        }
        ...
        function sendPdf() {
        	...
        	$.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/base/pdfEmail.do",
                 data: {
                	email: $("#email").val(),
        ...
        $(document).ready(function () {
            ...
            $("button#createExcel").click(createExcel);
        ...
        function saveJournalDetailRow() {
            if (lastSelectedJournalDetail) {
                $(journalDetailGrid).jqGrid(
                    ...
                    null,
                    "${pageContext.request.contextPath}/account/JournalDetail"
                );
        ...
        function searchCode(obj) {
        	...
        	/* alert($(journalDetailGrid).jqGrid("getLocalRow", lastSelectedJournalDetail).accountControlDescription); */
            $.ajax({
                ...
                success: function (jsonObj) {
                    // set the new data
                    $(codeGrid).jqGrid('setGridParam', {data: jsonObj});
        ...
        function createJournalGrid() {
            ...
            function saveSlip(selectedSlipNo) {
                ...
                $.ajax({
                    ...
                    success: function (jsonObj) {
                        ...
                        if (showSlipGrid())
                            $(slipGrid).jqGrid("setSelection", jsonObj);
                    }
                });
            }
            ...
            function saveJournal(selectedSlipNo) {
                ...
                $.ajax({
                    type: "PUT",
                    url: "${pageContext.request.contextPath}/account/Journal",
                    data: {
        ...
        function getAccountName(accountCode) {
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/account/Account",
                data: {
            ...
            $(slipGrid).jqGrid({
                ...
                ondblClickRow: function (id) {
                    ...
                    if (id && selectedSlipRow.slipStatus == REQUIRE_ACCEPT_SLIP) {
                        ...
                        $(slipGrid).jqGrid("editRow", id, true, null, null, "${pageContext.request.contextPath}/account/Slip",{type: "PUT"});
        ...
        function showSlipGrid(date) {
            ...
            $.ajax({
                type:"GET",
                url: "${pageContext.request.contextPath}/account/RangedSlipList",
                data: {
                    "from": $("#from").val(), /* ~부터 */
                    "to": $("#to").val() /* ~까지 */
                },
                ...
                success: function (jsonObj) {
                	...
                	if(jsonObj!=null){      		
                        // set the new data
                        $(slipGrid).jqGrid('setGridParam', {data: jsonObj});
                        // hide the show message
        ...
        function showJournalGrid(slipNo) {
            ...
            if (selectedSlipRow.slipNo !== NEW_SLIP_NO) {
                $.ajax({
                    type: "GET",
                    url: "${pageContext.request.contextPath}/account/SingleJournalList",
                    data: {
                        "slipNo": slipNo
                    },
                    dataType: "json",
                    success: function (jsonObj) {
                       jsonObj;
                        // set the new data
                        $(journalGrid).jqGrid('setGridParam', {data: jsonObj});
                        // refresh the grid
                        $(journalGrid).trigger('reloadGrid');
                        // hide the show message
                        $(journalGrid)[0].grid.endReq();
        ...
        function showJournalDetailSubGrid(parentRowId, parentRowKey) {
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/account/JournalDetailList",
                data: {
                ...
                success: function (jsonObj) {
                	console.log(jsonObj);
                    // set the new data
                    $(journalDetailGrid).jqGrid('setGridParam', {data: jsonObj});
                    // hide the show message
        ...
        function searchAccount() {
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/account/AccountListByName",
                data: {
                ...
                success: function (jsonObj) {
                    // set the new data
                    $(accountGrid).jqGrid('setGridParam', {data: jsonObj});
                    // hide the show message
			...
			<div class="row">
				...
				<div class="col-sm-6 form-row ml-0 mr-0">
					<div class="ml-auto">
						...
						<button type="button" id="createExcel"
							class="form-control form-control-sm btn btn-flat btn-outline-dark mb-3"
							style="width: 90px" disabled>EXCEL 보기</button>
